...
static struct process* processes[MODERNOS_MAX_PROCESSES] = {};
...
{
    int res = -ENOMEM;
    for(int i = 0; i < MODERNOS_MAX_PROGRAMS_ALLOCATIONS; i++)
    {
...
static void process_allocation_unjoined(struct process* process, void* ptr)
{
    for(int i = 0; i < MODERNOS_MAX_PROGRAMS_ALLOCATIONS; i++)
    {
        if(process->allocations[i].ptr == ptr)
...
void process_free(struct process* process, void* ptr)
{
    ...
    // It's not this processes pointer
    if(!allocation)
        return; 
    ...
    process_allocation_unjoined(process, ptr);
...
{
    int res = 0;
    paging_map_to(process->task->page_directory, (void*)MODERNOS_PROGRAM_VIRTUAL_ADDRESS, process->ptr, paging_align_address(process->ptr + process->size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
    return res;
}
...
int process_map_memory(struct process* process)
{
    ...
    // Map the stack of the user task
    paging_map_to(process->task->page_directory, (void*)MODERNOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack, paging_align_address(process->stack + MODERNOS_USER_PROGRAM_STACK_SIZE), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
...
{
    for(int i = 0; i < MODERNOS_MAX_PROCESSES; i++)
    {
        if(processes[i] == 0)
            return i;
    }
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot)
{
    ...
    program_stack_ptr = kzalloc(MODERNOS_USER_PROGRAM_STACK_SIZE);
