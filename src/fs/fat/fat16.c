...
#define MODERNOS_FAT16_SIGNATURE 0x29
#define MODERNOS_FAT16_FAT_ENTRY_SIZE 0x02
#define MODERNOS_FAT16_BAD_SECTOR 0xFF7
#define MODERNOS_FAT16_UNUSED 0x00
...
struct fat_private
{
    ...
    // Used in situation where we stream the directroy
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if(diskstreamer_seek(stream, directory_start_pos) != MODERNOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    while(1)
    {
        if(diskstreamer_read(stream, &item, sizeof(item)) != MODERNOS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if(diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != MODERNOS_ALL_OK)
    {
        res = -EIO;
        goto err_out;
    }

    if(diskstreamer_read(stream, dir, root_dir_size) != MODERNOS_ALL_OK)
    {
        res = -EIO;
        goto err_out;
    }
...
int fat16_resolve(struct disk* disk)
{
    ...
    if(diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != MODERNOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    if(fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != MODERNOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    ...
    for(int i = 0; i < clusters_ahead; i++)
    {
        ...
        {
            // We are at the last entry of the fat item
            res = -EIO;
            goto out;
        }

        // Sector is marked as bad?
        if(entry == MODERNOS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    for(int i = 0; i < directory->total; i++)
    {
        ...
        {
            // Found the fat item, let's create it
