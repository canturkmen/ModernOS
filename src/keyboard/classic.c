...
uint8_t classic_keyboard_scan_code_to_char(uint8_t scancode)
{
    ...
    if(keayboard_get_caps_lock(&classic_keyboard) == KEYBOARD_CAPS_LOCK_OFF)
    {
        if(c >= 'A' && c <= 'Z')
            c += 32;
    }
...
void classic_keyboard_handle_interrupt()
{
    ...
    {
        KEYBOARD_CAPS_LOCK_STATE old_state = keayboard_get_caps_lock(&classic_keyboard);
        keyboard_set_capslock(&classic_keyboard, old_state == KEYBOARD_CAPS_LOCK_ON ? KEYBOARD_CAPS_LOCK_OFF : KEYBOARD_CAPS_LOCK_ON);
    }

    uint8_t c = classic_keyboard_scan_code_to_char(scancode);
    if(c != 0) 
        keyboard_push(c);
