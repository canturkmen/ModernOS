...
{
    int i = 0;
    for(i = 0; i < MODERNOS_MAX_FILESYSTEMS; i++)
    {
        if(filesystems[i] == 0)
...
{
    int res = -ENOMEM;
    for(int i = 0; i < MODERNOS_MAX_FILEDESCRIPTORS; i++)
    {
...
{
    if(fd <= 0 || fd >= MODERNOS_MAX_FILEDESCRIPTORS)
        return 0;
...
struct filesystem* fs_resolve(struct disk* disk)
{
    ...
    for(int i = 0; i < MODERNOS_MAX_FILESYSTEMS; i++)
    {
